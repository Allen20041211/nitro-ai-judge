"""
model_v11.py — + sentence position, text-type dummies, Bayesian word smoothing,
                 text-level target features, 3rd LGB variant
Improvements over v10:
  - Sentence position (sent_pos, is_sent_start): strong psycholinguistic signal
  - Text-type one-hot (is_lit, is_enc, etc.): helps enc_wikimoon fold
  - Text-level target stats per fold (text_mean_trt, text_skip_rate)
  - Bayesian-smoothed word stats (shrink low-count toward global mean)
  - Participant reading consistency (part_cv)
  - 3rd LGB variant (different hyperparams for more diversity)
"""
import argparse, os
import pandas as pd
import numpy as np
...
parser.add_argument('--output',     default='/Users/renyihuang/NLP/submission_v11.csv')
...
# ============================================================
# 2. BERT surprisal cache
# ============================================================
if not os.path.exists(args.bert_cache):
    print("ERROR: BERT cache not found. Run first:")
...
gbs = raw_train['bert_surprisal'].median()
raw_train['bert_surprisal'] = raw_train['bert_surprisal'].fillna(gbs)
raw_test['bert_surprisal']  = raw_test['bert_surprisal'].fillna(gbs)
print("BERT surprisal merged. Mean: {:.2f}".format(raw_train['bert_surprisal'].mean()))

# ============================================================
# 3. Sentence position features (structural, no leakage)
# ============================================================
def add_sentence_features(df):
    """Detect sentence boundaries via punctuation; compute sent_pos per word_key."""
    ...
    for text, grp in uniq.groupby('text'):
        ...
        sent_id, pos_in = 0, 0
        for i, (w, k) in enumerate(zip(orig, keys)):
            records.append({'word_key': k, 'sent_id': sent_id, 'sent_pos': pos_in})
    ...
    df['sent_id']          = df['sent_id'].fillna(0)
    df['is_sent_start']    = (df['sent_pos'] == 0).astype(float)
    df['is_sent_start2']   = (df['sent_pos'] <= 1).astype(float)
...
# Text type mapping
TEXT_TYPES = ['lit','popsci','enc','ins','arg']
...
def add_structural_features(df):
    ...
    # Text type dummies
    tt = df['text_type'].fillna('other')
    for t in TEXT_TYPES:
        df[f'is_{t}'] = (tt == t).astype(float)
    return df
...
# ============================================================
# 5. N-gram LM surprisal (within-fold, no leakage)
# ============================================================
def build_lm(df):
...
# ============================================================
BAYES_K = 10  # shrinkage strength for Bayesian smoothing
...
def compute_target_tables(df):
    ...
    # Text-level aggregates (per specific text, not text_type)
...
def apply_tf(df, T):
    ...
    df['text_skip_rate']   = df['text_skip_rate'].fillna(gsr)
    df['text_nonzero_mean']= df['text_nonzero_mean'].fillna(gnm)
    df['text_rel_speed']   = df['text_mean_trt'] / gm
    df['part_x_text_rel']  = df['part_mean'] * df['text_rel_speed']
    ...
    # Sentence position interactions
...
FEATURES = [
    ...
    # Sentence position (NEW)
    'sent_pos','sent_pos_log','is_sent_start','is_sent_start2','sent_x_part','sent_x_bert',
    # Text type dummies (NEW)
    'is_lit','is_popsci','is_enc','is_ins','is_arg',
    ...
    # Text-level features (NEW)
    'text_mean_trt','text_median_trt','text_std_trt','text_skip_rate','text_nonzero_mean',
    'text_rel_speed','part_x_text_rel',
...
META_FEATS = [
    ...
    'bert_surprisal','surp_bi','wf_mean','wl_mean','wl_mean_bayes','tt_mean',
    'is_sent_start','text_mean_trt','text_skip_rate',
    'is_lit','is_popsci','is_enc','is_ins','is_arg',
]
...
lgb_p3 = dict(n_estimators=4000,learning_rate=0.015,num_leaves=255,min_child_samples=30,
               reg_alpha=0.05,reg_lambda=0.2,subsample=0.75,colsample_bytree=0.75,
...
oof_lgb=np.zeros(n); oof_lgb2=np.zeros(n); oof_lgb3=np.zeros(n)
...
print("="*70)
print("Cross-Validation (anti-leakage + BERT + sentence features)")
print("="*70)
...
for fold, (tr_idx, val_idx) in enumerate(gkf.split(raw_train, y_all, groups)):
    ...
    mA3 = lgb.LGBMRegressor(**lgb_p3); mA3.fit(X_tr,y_tr,eval_set=[(X_val,y_val)],callbacks=cb)
    pA3 = mA3.predict(X_val); oof_lgb3[val_idx]=pA3
    ...
    p_ens = 0.30*pA + 0.15*pA2 + 0.15*pA3 + 0.20*pB + 0.12*pC + 0.08*pD
...
X_meta = np.column_stack([oof_lgb,oof_lgb2,oof_lgb3,oof_xgb,oof_cat,oof_hgb,oof_skip,oof_hurdle,oof_me])
...
fA3 = lgb.LGBMRegressor(**lgb_p3); fA3.fit(X_full,y_full,callbacks=cb)
...
pA_t=fA.predict(X_test); pA2_t=fA2.predict(X_test); pA3_t=fA3.predict(X_test)
pB_t=fB.predict(X_test); pC_t=fC.predict(X_test)
pD_t=fD.predict(X_test); sk_t=fE.predict_proba(X_test)[:,1]
hurdle_t=(1-sk_t)*pA_t

X_mt = np.column_stack([pA_t,pA2_t,pA3_t,pB_t,pC_t,pD_t,sk_t,hurdle_t,
                         te_full[META_FEATS].fillna(0).values])
mp   = meta.predict(X_mt)
ens_t= 0.30*pA_t+0.15*pA2_t+0.15*pA3_t+0.20*pB_t+0.12*pC_t+0.08*pD_t
