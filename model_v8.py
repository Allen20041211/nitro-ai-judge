"""
model_v8.py — Anti-leakage + fixed ensemble + stronger features
Changes from v7:
  - Hurdle removed from direct ensemble (it was hurting score)
  - Better Romanian morphological features
  - Syllable count approximation
  - Optimized ensemble weights (LGB heavy)
  - Meta-model gets hurdle + skip as features only
"""
import argparse
import pandas as pd
import numpy as np
...
parser.add_argument('--output', default='/Users/renyihuang/NLP/submission_v8.csv')
...
# ============================================================
# Non-target features (external data / structure only)
# ============================================================
...
# Romanian common prefixes/suffixes (morphological complexity)
RO_PREFIXES = ['ne','re','des','de','în','sub','supra','anti','pre','pro']
RO_SUFFIXES_NOUN = ['ului','ilor','elor','ului','iile','iile','itor','ătoare','ătorului']
RO_SUFFIXES_VERB = ['ează','esc','ăsc','ști','ete','ăm','ați','esc','esc']
...
def count_syllables_ro(word):
    """Approximate Romanian syllable count by counting vowel groups."""
    w = str(word).lower()
    vowels = set('aeiouăâî')
...
def add_structural_features(df):
    ...
    # Syllables (reading time correlates strongly with syllable count)
    ...
    # wordfreq (external corpus — no leakage)
...
# ============================================================
# Target-dependent tables — computed per fold (anti-leakage)
# ============================================================
def compute_target_tables(df):
    ...
    p['part_nonzero_mean'] = (nz.groupby('participant_id')['answer'].mean()
                              if len(nz) else pd.Series(dtype=float))
    T['participant_stats'] = p

    T['part_texttype'] = (df.groupby(['participant_id','text_type'])['answer'].mean()
                          .rename('part_tt_mean').reset_index())
    T['part_tt_skip']  = (df.groupby(['participant_id','text_type'])
                          .apply(lambda x: (x['answer']==0).mean())
...
FEATURES = [
    ...
    # Word frequency (strongest generalizable signal)
    'zipf_ro','zipf_en','zipf_max','freq_log',
    'is_unknown_ro','is_rare','is_very_rare','is_common',
    'zipf_x_len','zipf_x_syl',
    # n-gram
    ...
    'zipf_x_part','zipf_x_skip','rare_x_part','freq_x_syl','difficulty',
]
...
for fold, (tr_idx, val_idx) in enumerate(gkf.split(raw_train, y_all, groups)):
    ...
    # Ensemble: simple average of regressors (no hurdle — hurdle goes to meta only)
...
print("Meta (in-sample, reference only): {:.2f}".format(eval_metric(y_all, meta_model.predict(X_meta))))
